...
def format_code_for_telegram(text):
    """Format code blocks specifically for Telegram display"""
    import re
    ...
    def replace_code_block(match):
        ...
        # Telegram doesn't support class attribute, use simple <pre>
        return f'<pre>{escaped_code}</pre>'

    # Replace code blocks with language (fixed regex without escaping)
    text = re.sub(r'\`\`\`(\w+)?\n?(.*?)\n?\`\`\`', replace_code_block, text, flags=re.DOTALL)

    # Replace inline code (fixed regex without escaping)
    text = re.sub(r'`([^`\n]+)`', lambda m: f'<code>{escape_code_content(m.group(1))}</code>', text)
